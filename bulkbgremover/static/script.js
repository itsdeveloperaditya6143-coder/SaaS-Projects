...
function processSingle(file) {
    ...
    fetch("/remove-stream", { method: "POST", body: formData })
        .then((response) => {
...
function processBulk(files) {
    ...
    fetch("/remove-bulk-stream", { method: "POST", body: formData })
        .then((response) => {
            const reader = response.body.getReader();
            const decoder = new TextDecoder();
            let buffer = "";
...
function showEditor(result) {
    ...
    downloadJpg.href = result.download_url;
    ...
    imgProcessed.src = result.download_url;
}
...
function renderBulkResults(bulkResult) {
    ...
    currentResults.forEach((r) => {
        ...
        card.innerHTML = `
            <div class="result-thumb"><img src="${r.download_url}" alt="${r.original}" loading="lazy"></div>
            <div class="result-meta">
                <span class="result-name" title="${r.original}">${r.original}</span>
                <span class="result-size">${formatBytes(r.size)}</span>
            </div>
        `;
        card.addEventListener("click", () => window.open(r.download_url, "_blank"));
        resultsGrid.appendChild(card);
    });
    downloadAllBtn.style.display = successCount > 1 ? "inline-flex" : "none";
}

downloadAllBtn.addEventListener("click", () => { if (currentZipUrl) window.location.href = currentZipUrl; });
